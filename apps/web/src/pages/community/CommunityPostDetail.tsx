...
import { t } from '../../theme';
import { ArrowLeft, MessageSquare, ThumbsUp, Send, CheckCircle2, Sparkles, AlertCircle, Loader2 } from 'lucide-react';
...
const CommunityPostDetail = () => {
  ...
  const replyMutation = useMutation({
    mutationFn: (content: string) => apiClient.post(`/community/posts/${id}/comments`, { content }),
    onSuccess: () => {
      setReply('');
        ...
        <div className="space-y-6 mb-12">
          {comments?.map((comment: any) => (
            ...
            >
              ...
              <div className="prose prose-invert max-w-none text-sm text-foreground/80 mb-6 whitespace-pre-wrap leading-relaxed">
                {comment.content}
              </div>
          ...
          <div className="bg-card border border-border p-4 md:p-6 rounded-[2rem] shadow-sm">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <textarea
                ...
                className="flex-1 bg-background border border-border rounded-xl px-4 py-3 font-medium text-sm text-foreground focus:outline-none focus:border-primary/50 resize-none"
                rows={3}
              />
              <button 
                onClick={() => replyMutation.mutate(reply)}
                disabled={!reply.trim() || replyMutation.isPending}
                className="bg-foreground text-background px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-primary hover:text-brand-navy transition-all disabled:opacity-50 shrink-0 h-fit"
              >
